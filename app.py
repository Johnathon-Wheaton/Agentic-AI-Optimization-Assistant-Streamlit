    ...
    if not selected_key or not selected_model:
        ...
        if st.session_state.in_progress == False:
            if st.session_state.chat_id == 1:
                ...
                st.session_state.agents = (consultant, user_proxy,  None, None, None)
                ...
                user_proxy = TrackableUserProxyAgent(
                    name="User_proxy",
                    system_message="A human. Only run code provided by the coder. Never run code provided by the CodeCritic.",
                    human_input_mode="NEVER",
                ...
                st.session_state.agents = (None, user_proxy, coder,  checker, code_critic)
        
        consultant, user_proxy, coder, checker, code_critic = st.session_state.agents
        ...
        if not st.session_state.in_progress and st.session_state.chat_id <3:
